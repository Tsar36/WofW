...
var VKI = function(customConfig, layout, deadKeys, keyInputCallback) {
  ...
  this.VKI_numberPad = config.numberPad !== undefined ? config.numberPad : true;  // Allow user to open and close the number pad
  ...
  this.VKI_size = config.size >=1 && config.size <= 5 ? config.size : 1;  // Default keyboard size (1-5)
  ...
  this.keyboardLocation = {
      UNDER_LEFT: "under-left",
      UNDER_CENTER: "under-center",
      UNDER_RIGHT: "under-right"
  };
  this.VKI_EX_keyboardLocation = config.keyboardLocation || this.keyboardLocation.UNDER_CENTER;
  
  this.VKI_EX_customKeyboardInputInitiator = config.customKeyboardInputInitiator || false;
  // Here can be set HTML string that will represent button to invoke virtual keyboard
  this.VKI_EX_keyboardInputInitiator = config.keyboardInputInitiator || "Keyboard";
  ...
  self.attachVki = function(elem) {
      ...
      if (!self.VKI_EX_customKeyboardInputInitiator) {
          ...
          // div is here for converting html into DOM object
          var div = document.createElement('div');
          div.innerHTML = self.VKI_EX_keyboardInputInitiator;
          // retrieve convertation result
          var keybut = div.firstChild;
          keybut.elem = elem;
          keybut.onclick = function (e) {
              e = e || event;
              if (e.stopPropagation) { e.stopPropagation(); } else e.cancelBubble = true;
  ...
  this.VKI_position = function(force) {
    ...
    if (self.VKI_target) {
      ...
      if (place || force === true) {
        ...
        self.VKI_keyboard.style.top = iPos[1] + self.VKI_target.clientHeight + 3 + "px";
        // Too heavy to fight with this keyboard location calculation to make it work with modal windows
        // = iPos[1] - ((self.VKI_target.keyboardPosition == "fixed" && !self.VKI_isIE && !self.VKI_isMoz) ? sDis[1] : 0) + fudge + "px";
        switch (self.VKI_EX_keyboardLocation) {
            case self.keyboardLocation.UNDER_RIGHT: iPos[0] += self.VKI_target.clientWidth; break;
            case self.keyboardLocation.UNDER_CENTER: break;
            case self.keyboardLocation.UNDER_LEFT: iPos[0] -= self.VKI_keyboard.clientWidth; break;
        }
        self.VKI_keyboard.style.left = Math.max(0, iPos[0]) + "px";
        // = Math.max(10, Math.min(wDim[0] - self.VKI_keyboard.offsetWidth - 25, iPos[0])) + "px";
        if (self.VKI_isIE6) {
...
.service('ngVirtualKeyboardService', ['VKI_CONFIG', function(VKI_CONFIG) {
    ...
    return {
        attach: function(element, config, inputCallback) {
            ...
            config.customKeyboardInputInitiator = VKI_CONFIG.customKeyboardInputInitiator || false;
            config.keyboardInputInitiator = VKI_CONFIG.keyboardInputInitiator || "";

            var vki = new VKI(config, VKI_CONFIG.layout, VKI_CONFIG.deadkey, inputCallback);
            vki.attachVki(element);
        },

        getKeyboardLayout: function(kt){
            if (kt && !VKI_CONFIG.layout[kt]) {
                for (var i in VKI_CONFIG.layout) {
                    if (VKI_CONFIG.layout[i].lang && VKI_CONFIG.layout[i].lang[0]
                            && VKI_CONFIG.layout[i].lang[0].length >= 2) {
                        if (VKI_CONFIG.layout[i].lang[0].substring(0, 2).toLowerCase()
                            == kt.substring(0, 2).toLowerCase()) {
                            kt = i;
                            break;
                        }
                    }
            ...
            if (!VKI_CONFIG.layout[kt]) kt = undefined;
            return kt;
        }
	};
}])
.directive('ngVirtualKeyboard', ['ngVirtualKeyboardService', '$timeout', '$injector',
	...
	return {
		...
		link: function(scope, elements, attrs, ngModelCtrl) {
			...
			attrs.$observe('ngVirtualKeyboard', function (value) {
			    scope.config = JSON.parse(value);

			    scope.config.kt = ngVirtualKeyboardService.getKeyboardLayout(scope.config.kt);
			    var vki = ngVirtualKeyboardService.attach(elements[0], scope.config, function () {
			        $timeout(function () {
